...
fn main() -> Result<()> {
    ...
    let mut unique_id = LOOPBACK;
    let mut loopback_id_queue: VecDeque<usize> = VecDeque::new();
            ...
            match event.token().0 {
                SERVER | LOOPBACK => loop {
                    ...
                    let id = Token(if is_loopback {
                        loopback_id_queue.pop_front().unwrap()
                    } else {
                        unique_id += 1;
                        unique_id
                    });
                ...
                cli_id => {
                    if let Err(_e) = handle_client(
                        poll.registry(),
                        &mut connections,
                        event,
                        &mut unique_id,
                        &mut loopback_id_queue,
                    ) {
                        let mut peer_id = 0;
                        if let Some(mut cli) = connections.remove(&Token(cli_id)) {
                        ...
                        if peer_id != 0 {
                            if let Some(mut cli) = connections.remove(&Token(peer_id)) {
                                let _ = poll.registry().deregister(&mut cli.socket);
                                if !cli.is_loopback {
...
fn handle_client(
    registry: &Registry,
    connections: &mut HashMap<Token, Client>,
    event: &Event,
    unique_id: &mut usize,
    loopback_id_queue: &mut VecDeque<usize>,
) -> Result<()> {
    let cli_id = &event.token();
    let cli = match connections.get_mut(cli_id) {
        Some(c) => c,
    ...
    if event.is_readable() {
        ...
        if cli.peer_id == 0 {
            ...
            let mut loopback_sock = match TcpStream::connect("127.0.0.1:4433".parse()?) {
                Ok(s) => s,
                Err(e) => return Err(From::from(format!("Failed to connect to loopback : {}", e))),
            };
            *unique_id += 1;
            loopback_id = *unique_id;
            *unique_id += 1;
            let peer_loopback_id = *unique_id;
            loopback_id_queue.push_back(peer_loopback_id);
            ...
            let mut peer_sock = match TcpStream::connect("192.168.2.222:443".parse()?) {
                Ok(s) => s,
                Err(e) => return Err(From::from(format!("Failed to connect to peer : {}", e))),
            };
            *unique_id += 1;
            forward_id = *unique_id;
        ...
        //Forward to loopback
        if let Some(peer) = connections.get_mut(&Token(loopback_id)) {
        ...
        if let Some(peer) = connections.get_mut(&Token(forward_id)) {
